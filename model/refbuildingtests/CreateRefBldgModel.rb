...
spaces.each do |space|
  if not space.partofTotalFloorArea # Brent said this isnt' very relyable on typical IDF models. Will want to map just like any other space type)
...
not_in_area_spaces.each do |not_in_area_space|
  ...
  surfaces.each do |surface|
    ...
    if surface.surfaceType == "Floor" and surface.outsideBoundaryCondition == "Surface" # need to add in or statement to catch adiabatic
  ...
  if roof_conductance.size > 0  and floor_conductance.size > 0
...
non_plenumAttic_stories.each do |story|
  story_spaces = story.spaces
    ...
    if min_multiplier > 1
      puts "> Thermal Zones on #{story.name.to_s} have a minimum multiplier of #{min_multiplier}"
    end
    adjusted_story_count = adjusted_story_count + multipliers.min
end
...
if plenum_flag
  ...
  plenum_floor_layers.reverse_each do |plenum_floor_layer|
    plenum_floor_layers_counter = plenum_floor_layers_counter - 1   
  ...
  plenum_roof_layers.reverse_each do |plenum_roof_layer|
    plenum_roof_layers_counter = plenum_roof_layers_counter - 1   
...
if attic_flag == true
  ...
  template_constructions.each do |template_construction|
    ...
    if found_roof_flag == true and found_roof_flag == true
      break
    end
  end  
