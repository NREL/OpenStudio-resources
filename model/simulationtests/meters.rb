...
#      nmeter_custom = OpenStudio::Model::MeterCustom.new(model)
...
#      n  # A keyvar group
#      n  meter_custom.addKeyVarGroup(hc.name.to_s, variable_name)
#      nend
#      n
#      n# Create an OutputMeter to output the meter to the SQL file
#      nmeter = OpenStudio::Model::OutputMeter.new(model)
...
#      nmeter_customdecrement = OpenStudio::Model::MeterCustomDecrement.new(model, source_meter_name)
...
#      n# Create an OutputMeter to output the meter to the SQL file
#      nmeter = OpenStudio::Model::OutputMeter.new(model)
#      nmeter.setName(meter_customdecrement.name.to_s)
#      nmeter.setReportingFrequency(reporting_frequency)
...
model.getAirTerminalSingleDuctVAVReheats.each do |atu|
  # Get the reheat coil
  hc = atu.reheatCoil.to_CoilHeatingWater.get
  # A keyvar group
  meter_custom.addKeyVarGroup(hc.name.to_s, variable_name)
end
...
meter_customdecrement = OpenStudio::Model::MeterCustomDecrement.new(model, source_meter_name)
...
# Create an OutputMeter to output the meter to the SQL file
meter = OpenStudio::Model::OutputMeter.new(model)
meter.setName(meter_customdecrement.name.to_s)
meter.setReportingFrequency(reporting_frequency)
