...
surfaces = sorted(
    [s for s in model.getSurfaces() if s.outsideBoundaryCondition() == "Outdoors"], key=lambda s: s.nameString()
)
...
construction_set = sorted(model.getDefaultConstructionSets(), key=lambda z: z.nameString())[0]
...
if openstudio.VersionString(openstudio.openStudioVersion()) > openstudio.VersionString("2.7.1"):
