...
def curve_quadratic(model, c_1constant, c_2x, c_3xPOW2, minx, maxx, miny=None, maxy=None):
...
def curve_triquadratic(
    ...
    curve = openstudio.model.CurveTriquadratic(model)
...
def curve_table_lookup_cool_charge_cool_cap_ft(model):
    curve = openstudio.model.TableLookup(model)

    ind_var_1 = openstudio.model.TableIndependentVariable(model)
    ...
    ind_var_2 = openstudio.model.TableIndependentVariable(model)
    ...
    ind_var_3 = openstudio.model.TableIndependentVariable(model)
...
coil = openstudio.model.CoilCoolingDXSingleSpeedThermalStorage(m)
...
cool_cap_ft = curve_biquadratic(
    m, 0.9712123, -0.015275502, 0.0014434524, -0.00039321, -0.0000068364, -0.0002905956, -100, 100, -100, 100
)
constant_cubic = curve_cubic(m, 1, 0, 0, 0, -100, 100)
cool_eir_ft = curve_biquadratic(
    m, 0.28687133, 0.023902164, -0.000810648, 0.013458546, 0.0003389364, -0.0004870044, -100, 100, -100, 100
)
cool_plf_fplr = curve_quadratic(m, 0.90949556, 0.09864773, -0.00819488, 0, 1, 0.7, 1)
cool_shr_ft = curve_biquadratic(
    m, 1.3294540786, -0.0990649255, 0.0008310043, 0.0652277735, -0.0000793358, -0.0005874422, 24.44, 26.67, 29.44, 46.1
)
...
cool_charge_cool_cap_ft = curve_triquadratic(
    m,
    0.9712123,
    0.0014434524,
    -0.015275502,
...
cool_charge_cool_eir_ft = curve_triquadratic(
    m,
    0.28687133,
    -0.000810648,
    0.023902164,
...
discharge_cap_ft = curve_biquadratic(
    m, -0.561476105575098, 0.133948946696947, -0.0027652398813276, 0.0, 0.0, 0.0, -100, 100, -100, 100
)
...
discharge_shr_ft = curve_biquadratic(
    m,
    -76.3312028672366,
    3.69083877577677,
    0.00402614182268047,
...
# EMSControlled
sensor = openstudio.model.EnergyManagementSystemSensor(m, "Cooling Coil Ice Thermal Storage End Fraction")
sensor.setName(f"{coil.nameString()} s")
sensor.setKeyName(coil.nameString())

schedule = openstudio.model.ScheduleConstant(m)
schedule.setValue(5)

sensor_2 = openstudio.model.EnergyManagementSystemSensor(m, "Schedule Value")
sensor_2.setName(f"{schedule.nameString()} s")
sensor_2.setKeyName(schedule.nameString())

actuator = openstudio.model.EnergyManagementSystemActuator(
    coil, "Coil:Cooling:DX:SingleSpeed:ThermalStorage", "Operating Mode"
)
actuator.setName(f"{coil.nameString()} opmode")

program = openstudio.model.EnergyManagementSystemProgram(m)
program.setName(f"{coil.nameString()} control")
program.addLine(f"Set {actuator.nameString()} = {sensor_2.nameString()}")
program.addLine(f"If ({actuator.nameString()} == 2)")
program.addLine(f"If ({sensor.nameString()} > 0.99)")
program.addLine(f"Set {actuator.nameString()} = 1")
program.addLine("EndIf")
program.addLine("EndIf")
program.addLine(f"If ({actuator.nameString()} == 5)")
program.addLine(f"If ({sensor.nameString()} < 0.01)")
program.addLine(f"Set {actuator.nameString()} = 1")
program.addLine("EndIf")
program.addLine("EndIf")

pcm = openstudio.model.EnergyManagementSystemProgramCallingManager(m)
pcm.setName(f"{coil.nameString()} pcm")
