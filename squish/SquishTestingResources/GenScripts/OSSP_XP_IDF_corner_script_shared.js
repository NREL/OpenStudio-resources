...
function universalcode()
{
    ...
    var sourcepath = ('C:\\OpenStudio_Squish_SVN\\squish\\suite_OSSP_XP_IDF_corner\\') // this is set by test programmer for each test suite
    var exportpath = (nonsvnpath + '\\SquishTestingExports\\OSSP_XP_IDF_corner-exports\\' + vpname + '.osm'); // user or cmake needs to make these folders
    ...
    type(waitForObject(":SketchUp.Yes_Button",14400000), "<Tab>");// 4 hour delay allowed for file opening
    ...
    waitFor("object.exists(':OpenStudio:  Input File Errors And Warnings_Edit')", 900000);
    ...
    waitFor("object.exists(':four_scenes.skp - SketchUp Pro_MenuBar')", 14400000);
    ...
    type(waitForObject(":_Edit"), nonsvnpath + '\\SquishTestingExports\\OSSP_XP_IDF_corner-exports\\' + vpname + '.jpg');
    type(waitForObject(":_Edit"), "<Return>");
    ...
    var dTime = new Date();
    var millisec_a = dTime.getTime();
    type(waitForObject(":SketchUp.Yes_Button",3600000), "<Tab>");// 60 minute delay allowed for file opening
